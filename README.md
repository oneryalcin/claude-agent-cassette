...
- **The `system/init` frame is a leak of its own**: it enumerates the recording
  environment (slash commands, plugins with paths, skills, agents, MCP servers, tools).
  Best fix: record under an isolated config dir (`env={"CLAUDE_CONFIG_DIR": <fresh tmp
  dir>}`, as every example recorder does). After-the-fact: `scrub_init_inventory(tape)`
  blanks those inventory lists — replay never reads them, so it is decision-preserving
  by construction.
...
| `scrub_init_inventory(tape)` | blank the `system/init` environment inventory (the operator/company fingerprint) |
